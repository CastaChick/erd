...
export function renderIndex(entries: DiagramEntry[], warnings: string[]): string {
  const lines = ['# Database ER Diagrams', '', 'Generated from PostgreSQL schema metadata.', '',
    '## Overview', '', '[Overview](./overview.mmd) · [Graph metadata](./graph.json)', '',
  ...
  for (const { community: c, file } of entries) {
    lines.push(`### ${safe(c.id)} — ${safe(c.hub)}${c.isolated ? ' (isolated)' : ''}`, '',
      ...
      `[Diagram](./${file})`, '', `Tables: ${c.tables.map(safe).join(', ')}`, '',
